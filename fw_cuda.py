...
class FwCudaSwitch13(app_manager.RyuApp):
    ...
    def BuildNixVector(self, srcSwitch, dstSwitch, links, sdnNix):
        ...
        #self.logger.info("src=%d, dst=%d, N=%d", src, dst, N)
        if self.next_array[src * N + dst] == None:
            return False

        while src != dst:
            currSwitch = api.get_switch(self, src)[0]
            nextLink = [link for link in links if link.src.dpid == currSwitch.dp.id and link.dst.dpid == self.next_array[src * N + dst]][0]
            sdnNix.append((currSwitch, nextLink.src.port_no))
            src = self.next_array[src * N + dst]
        return True
